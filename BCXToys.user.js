...
const BCXToysIgnoreMsgTypes = new Set(['Status', 'Hidden']);
...
function BCXToysSearchMsgDictionary(msg, key) {
...
(async function () {
    ...
    ServerSocket.on("ChatRoomMessage", async (data) => {
        ...
        // Activity affecting player
        if (data.Type == 'Activity' && BCXToysSearchMsgDictionary(data, 'TargetCharacter')?.MemberNumber === Player.MemberNumber) {
            var activityGroup = BCXToysSearchMsgDictionary(data, 'ActivityGroup')?.Text;
